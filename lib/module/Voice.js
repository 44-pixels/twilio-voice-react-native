...
import { settleNativePromise } from './utility/nativePromise'; // >>> FORK KAR-787 — see type/CallSound.ts
...
export class Voice extends EventEmitter {
  ...
  constructor() {
    ...
    _defineProperty(this, "_handlePushTokenChanged", nativeVoiceEvent => {
      ...
      if (Platform.OS === 'android' && this.listenerCount(Voice.Event.PushTokenChanged) > 0) {
  ...
  async _consumePendingPushToken() {
    const token = await settleNativePromise(NativeModule.voice_consumePendingPushToken());
  ...
  async completeCallbackRequest(requestId) {
    return settleNativePromise(NativeModule.voice_clearCallbackRequest(requestId));
  }
  /**
  ...
  async setCallSoundSettings(settings) {
    ...
    await settleNativePromise(NativeModule.voice_setCallSoundSettings(settings.ringtone.mode, ringtoneSoundId, settings.callEnded.mode));
  }
