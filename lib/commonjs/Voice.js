...
class Voice extends _eventemitter.EventEmitter {
  ...
  constructor() {
    ...
    _defineProperty(this, "_handleCallbackRequested", nativeVoiceEvent => {
      if (nativeVoiceEvent.type !== _constants.Constants.VoiceEventCallbackRequested) {
        throw new Error('Incorrect "voice#callbackRequested" handler called for type ' + `"${nativeVoiceEvent.type}".`);
      }
      ...
      this.emit(Voice.Event.CallbackRequested, {
        requestId,
        handle
      });
    });
    ...
    _defineProperty(this, "_handlePushTokenChanged", nativeVoiceEvent => {
      if (nativeVoiceEvent.type !== _constants.Constants.VoiceEventPushTokenChanged) {
        throw new Error('Incorrect "voice#pushTokenChanged" handler called for type ' + `"${nativeVoiceEvent.type}".`);
      }

      if (_common.Platform.OS === 'android' && this.listenerCount(Voice.Event.PushTokenChanged) > 0) {
    ...
    this._nativeEventHandler = {
      ...
      [_constants.Constants.VoiceEventCallbackRequested]: this._handleCallbackRequested,
      // <<< FORK
      ...
      [_constants.Constants.VoiceEventPushTokenChanged]: this._handlePushTokenChanged,
      // <<< FORK
  ...
  on(voiceEvent, listener) {
    const shouldConsumePushToken = voiceEvent === Voice.Event.PushTokenChanged && this.listenerCount(Voice.Event.PushTokenChanged) === 0;
    super.on(voiceEvent, listener);

    if (shouldConsumePushToken && _common.Platform.OS === 'android') {
      this._consumePendingPushToken();
    }
  ...
  addListener(voiceEvent, listener) {
    const shouldConsumePushToken = voiceEvent === Voice.Event.PushTokenChanged && this.listenerCount(Voice.Event.PushTokenChanged) === 0;
  ...
  async _consumePendingPushToken() {
    const token = await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_consumePendingPushToken());

    if (token !== null && this.listenerCount(Voice.Event.PushTokenChanged) > 0) {
  ...
  async completeCallbackRequest(requestId) {
    return (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_clearCallbackRequest(requestId));
  }
  /**
  ...
  async getInitialCallbackRequest() {
    return (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_getInitialCallbackRequest());
  } // <<< FORK
  ...
  async setCallSoundSettings(settings) {
    const ringtoneSoundId = settings.ringtone.mode === 'bundled' ? settings.ringtone.soundId : undefined;
    await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_setCallSoundSettings(settings.ringtone.mode, ringtoneSoundId, settings.callEnded.mode));
  }
  /** Return the persisted native call-sound settings. */


  async getCallSoundSettings() {
    return (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_getCallSoundSettings());
  }
  /** Return sounds installed by the Expo config plugin. */


  async getAvailableRingtones() {
    return (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_getAvailableRingtones());
  }
  /** Preview an installed call sound. */


  async previewCallSound(soundId) {
    await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_previewCallSound(soundId));
  }
  /** Stop the active call-sound preview, if any. */


  async stopCallSoundPreview() {
    await (0, _nativePromise.settleNativePromise)(_common.NativeModule.voice_stopCallSoundPreview());
  } // <<< FORK
...
(function (_Voice) {
  ...
  (function (Event) {
    ...
    Event["CallbackRequested"] = "callbackRequested";
    Event["Error"] = "error";
    Event["Registered"] = "registered";
    Event["Unregistered"] = "unregistered";
    Event["PushTokenChanged"] = "pushTokenChanged";
  })(Event || (Event = {}));
